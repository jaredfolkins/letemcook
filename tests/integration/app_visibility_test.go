	...
	SeriesTestWrapper(t, func(t *testing.T, instance *TestInstance) {
		...
		ChromeDPTestWrapperWithInstance(t, instance, func(ctx context.Context) {
			...
			t.Run("BravoCanSeeOwnApps", func(t *testing.T) {
				// Log out from the current session first using POST request
				baseURL := GetBaseURLForInstance(instance)
				logoutTasks := chromedp.Tasks{
					// Send POST request to logout endpoint to clear the session
					...
					chromedp.Sleep(2 * time.Second), // Wait for logout to complete
				}

				if err := chromedp.Run(ctx, logoutTasks); err != nil {
