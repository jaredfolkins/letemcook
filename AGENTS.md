...
* **Persistent Storage:** LEMC uses a local `data/<env>/` directory (on the host or container running LEMC) to store its SQLite database and configuration (it auto-initializes this on first run). The `<env>` portion is determined by the `LEMC_ENV` setting. This storage retains all cookbook definitions, user accounts, execution history, etc., across restarts.
...
*   Run the compiled binary directly:
    ...
    ./tmp/lemc
    ```
*   On the first run, LEMC will initialize the `data/<env>/` directory, create `data/<env>/.env`, run migrations, and start the server using the port configured for your environment (default development port: `http://localhost:5362`).
